...
> **이 파일의 약속**: 약 270줄 이하 유지 (±10줄). 상세는 분리 문서 링크. 검증 명령 SSOT는 §6.
> **용어**: **MUST/MUST NOT** = 위반 시 PR 거부 + AI 즉시 거부 · **SHOULD** = 합당한 사유 없이 위반 금지 (ADR로 예외 기록) · **MAY** = 자유.

---

## 0. SMART+ER Header (AI Agent Briefing)

> 채택 근거: ADR-0004 (SMART+ER 메타 프롬프트 채택). 3사(Claude/Codex/Gemini) 교차검증 합의안 v1.

- **S (Situation)**: 사내망+제한 외부 On-Prem K8s v1.34+ Pansori 프로젝트. 현재 P0(스펙·인터페이스 정의 단계). Coroot 순수 영감(ADR-0003), Apache-2.0. 대상 에이전트: Claude/Codex/Gemini + SubOrchestrator.
- **M (Mission)**: 상위 SSOT를 우선해 안전하게 작업하고 검증 가능한 결과만 보고한다. P0 종료 = ADR ≥`[TBD]`건·SPEC ≥`[TBD]`건·`modules/contracts/` 스켈레톤 동결, 기한 `[TBD]`, 검수자 = 모듈 오너 전원. (수치·기한은 ADR-0004에서 확정)
- **A (Action Steps)**: §1 Quickstart 라우터(분류 → 참조 → 수정 → 검증) 순서 준수. §1의 "확인 게이트 정책" 항목은 사용자 승인 대기.
- **R (Result)**: 응답 포맷 = `변경 요약 / 영향 범위 / 검증 결과 / 남은 리스크` 4항목. 작업 유형별 DoD(§1.2) 전부 충족.
- **T (Tone & Style)**: 출력=한국어 / 형식=간결한 Markdown / 근거=`파일:라인` / 금지=추측·hedging·이모지·서론·결론 반복. MUST/SHOULD/MAY 강도 준수.
- **E (Example)**: 권장 패턴 → `docs/templates/{adr,spec,contracts}.md` `[TBD: P1에서 생성]`. 안티패턴 → §4.
- **R (Resource)**: 충돌 시 §8의 7단계 우선순위. 모든 링크는 §9.
...
2) 작업 유형 선택 → 해당 행만 로딩 + 완료 기준(DoD) 충족:
   ┌─────────────────────┬─────────────────────────────────────┬──────────────────────────────────────────┐
   │ 작업 유형           │ 먼저 볼 문서                        │ 완료 기준 (Done = 모두 충족)             │
   ├─────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
   │ ADR 신규/수정       │ docs/30-adr/, 본 파일 §3·§4         │ 영향 SPEC 링크 ≥2 + 결정자 1명 + 대안 ≥1 │
   │ SPEC 신규/수정      │ docs/20-specs/, contracts-spec      │ EARS 형식 + acceptance ≥3개 + REQ-ID 유 │
   │ 인터페이스 변경     │ modules/contracts/ + 영향 모듈 SPEC │ buf generate 통과 + 메시지당 examples 1  │
   │ 새 모듈 구현(P1+)   │ 본 §5 진입점 표 → spec → contracts  │ test 커버리지 ≥85% + §6 검증 원샷 통과   │
   │ 버그 수정           │ 실패 테스트 작성 → 재현 → 패치      │ 회귀 테스트 추가 + 기존 테스트 0개 깨짐  │
   │ K8s API 사용        │ docs/30-adr/0002 (canonical 상태표) │ Tier 분류 명시 + capability gating 코드  │
   │ 보안/배포 표준      │ docs/20-specs/onprem-baseline-spec  │ PSA/NetworkPolicy 매니페스트 동봉        │
   │ 문서·번역 초안      │ docs/01-architecture.md + glossary  │ 용어 일관성(glossary 준수) + 링크 0개 깨짐│
   └─────────────────────┴─────────────────────────────────────┴──────────────────────────────────────────┘

3) 작업 후 SSOT 검증 (§6 명령 복사 실행):
   go build ./... && go test ./... && go vet ./... && golangci-lint run && govulncheck ./...

4) 확인 게이트 정책 (Confirmation Points) — One-Shot vs 사용자 확인의 경계:
   ┌─────────────────────────────────────────────┬──────────────┬────────────────────────────────────┐
   │ 상황                                        │ 정책          │ 근거                              │
   ├─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
   │ 새 외부 의존성 추가                         │ 사용자 확인必 │ ADR 필요 (contributing.md §1)      │
   │ public API / contracts 변경                 │ 사용자 확인必 │ 영향도 다모듈, SSOT 영향          │
   │ auth·RBAC·SSO 정책 변경                     │ 사용자 확인必 │ 보안 게이트                       │
   │ K8s API surface 변경 (Tier 1→2 등)          │ 사용자 확인必 │ ADR-0001/0002 영향               │
   │ ADR 신규 작성/기존 ADR 폐기                 │ 사용자 확인必 │ 거버넌스 SSOT                    │
   │ 시크릿/PII 처리 로직                        │ 사용자 확인必 │ 보안 게이트                       │
   │ 위 외 모든 작업 (SPEC 초안, 코드, 문서, 리뷰)│ One-Shot 진행 │  Opus 4.7 철학   │
   └─────────────────────────────────────────────┴──────────────┴────────────────────────────────────┘
   • One-Shot 진행 = 중간 확인 없이 작업 완료 후 결과+근거 보고
   • 사용자 확인必 = AskUserQuestion 한 번 호출 후 진행
   • 회색지대는 §7 위임 가이드의 "3사 병렬 교차검증" 사용
```
